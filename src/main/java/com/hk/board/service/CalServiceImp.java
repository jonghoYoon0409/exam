...
public class CalServiceImp {
	...
	public Map<String, Integer> makeCalendar(HttpServletRequest request){
		...
		int dayOfWeek=cal.get(Calendar.DAY_OF_WEEK); //1~7숫자중에 반환: 1은 일요일~ 7은 토요일
		...
		//한달에 대한 일정을 구하는 작업
		String id=request.getParameter("patientid");// session에 저장된 id값을 불러와야 함
		if(id==null) {
			id=(String)session.getAttribute("id");//session으로부터 id정보 가져오기			
		}else {
			session.setAttribute("patientid", id);
		}
		
		String yyyyMM=year+util.isTwo(month+"");//년월 6자리
		List<CalDto> clist=calViewList(id, yyyyMM); //calViewList 메서드를 호출해, 해당 연도와 월에 맞는 일정 목록을 가져와.
